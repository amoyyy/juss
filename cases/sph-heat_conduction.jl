...
"""    re = 100
    ...
    VARSALL = [:x, :y, :z, :h, :p, :rho, :m, :V, :T, :H,
                :aT, :vx, :vy, :vz, :ax, :ay, :az, :arho]
    VARS_FLUID_ADDITION = Symbol[]
    VARS_BOUNDARY_ADDITION = Symbol[]
###############################################################################
    # DEBUG
    ...
    function SPHApplication.create_equations(p::SPHApplication.Case)
        ...
        SPHEquations.add(equations, 2, SPHEquations.HeatConductionEquationT(:Fluid, :ALL, MaterialLib.Sodium(), MaterialLib.Sodium(), 0.00))
        return equations
    end
...
case = SPHApplication.Case("SPH-HeatConduction", "./cases/output")
