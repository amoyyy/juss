...
###############################################################################
    struct WCSPHTVDRK3Step <: IntegratorStep end
    ...
    """
    function initialize(p::WCSPHTVDRK3Step, d_idx, d_x0, d_y0, d_z0, d_x, d_y, d_z,
    ...
    function stage1(p::WCSPHTVDRK3Step, d_idx, d_x0, d_y0, d_z0, d_x, d_y, d_z,
               d_u0, d_v0, d_w0, d_u, d_v, d_w, d_rho0, d_rho,
    ...
    function stage2(p::WCSPHTVDRK3Step, d_idx, d_x0, d_y0, d_z0, d_x, d_y, d_z,
    ...
    function stage3(p::WCSPHTVDRK3Step, d_idx, d_x0, d_y0, d_z0, d_x, d_y, d_z,
               d_u0, d_v0, d_w0, d_u, d_v, d_w, d_rho0, d_rho, d_au, d_av,
               d_aw, d_ax, d_ay, d_az, d_arho, dt)
...
###############################################################################
# `SolidMechStep` struct
###############################################################################
    struct SolidMechStep <: IntegratorStep end
    """Predictor corrector Integrator for solid mechanics problems"""
    function initialize(p::SolidMechStep, d_idx, d_x0, d_y0, d_z0, d_x, d_y, d_z,
                   d_u0, d_v0, d_w0, d_u, d_v, d_w, d_rho0, d_rho,
    ...
    function stage1(p::SolidMechStep, d_idx, d_x0, d_y0, d_z0, d_x, d_y, d_z,
                  d_u0, d_v0, d_w0, d_u, d_v, d_w, d_rho0, d_rho, d_au, d_av,
    ...
    function stage2(p::SolidMechStep, d_idx, d_x0, d_y0, d_z0, d_x, d_y, d_z,
                  d_u0, d_v0, d_w0, d_u, d_v, d_w, d_rho0, d_rho, d_au, d_av,
...
###############################################################################
    ...
    """Integrator functionined in 'A transport velocity formulation for
    ...
    """
    function initialize(p::TransportVelocityStep, data::Dict{String,Vector{Float64}}, idx::Vector{Int}) end

    function stage1(p::TransportVelocityStep, d_idx, d_u, d_v, d_w, d_au, d_av, d_aw, d_uhat, d_auhat, d_vhat,
    ...
    function stage1(p::TransportVelocityStep, data::Dict{String,Vector{Float64}}, idx::Vector{Int},
                    dt::Float64)
        dtb2 = 0.5*dt

        # velocity update eqn (14)
        pool[:vx] += dtb2*pool[:ax]
        pool[:vy] += dtb2*pool[:ay]

        # advection velocity update eqn (15)
        d_uhat[d_idx] = pool[:vx] + dtb2*d_auhat[d_idx]
        d_vhat[d_idx] = pool[:vy] + dtb2*d_avhat[d_idx]

        # position update eqn (16)
        pool[:x] += dt*d_uhat[d_idx]
        pool[:y] += dt*d_vhat[d_idx]
    end

    function stage2(p::TransportVelocityStep, d_idx, d_u, d_v, d_w, d_au, d_av, d_aw, d_vmag2, dt)
        dtb2 = 0.5*dt
...
###############################################################################
# `AdamiVerletStep` struct
###############################################################################
    struct AdamiVerletStep <: IntegratorStep end
    ...
    """
    function initialize(p::AdamiVerletStep) end

    function stage1(p::AdamiVerletStep, d_idx, d_u, d_v, d_w, d_au, d_av, d_aw, d_x, d_y, d_z, dt)
        dtb2 = 0.5*dt
    ...
    function stage2(p::AdamiVerletStep, d_idx, d_u, d_v, d_w, d_au, d_av, d_aw, d_x, d_y, d_z,
...
###############################################################################
# `GasDFluidStep` struct
###############################################################################
    struct GasDFluidStep <: IntegratorStep end
    """Predictor Corrector integrator for Gas-dynamics"""
    function initialize(p::GasDFluidStep, d_idx, d_x0, d_y0, d_z0, d_x, d_y, d_z, d_h,
        ...
        # likewise, we set the functionault omega (grad-h) terms to 1 at
    ...
    function stage1(p::GasDFluidStep, d_idx, d_x0, d_y0, d_z0, d_x, d_y, d_z,
    ...
    function stage2(p::GasDFluidStep, d_idx, d_x0, d_y0, d_z0, d_x, d_y, d_z,
               d_u0, d_v0, d_w0, d_u, d_v, d_w, d_e0, d_e, d_au, d_av,
               d_alpha1, d_aalpha1, d_alpha10,
               d_alpha2, d_aalpha2, d_alpha20,
               d_aw, d_ae, dt)
    ...
    struct GSPHStep <: IntegratorStep end
    function stage1(p::GSPHStep, d_idx, d_x, d_y, d_z, d_u, d_v, d_w, d_e,
    ...
    struct ADKEStep <: IntegratorStep end
    """Predictor Corrector integrator for Gas-dynamics ADKE"""
    function initialize(p::ADKEStep, d_idx, d_x0, d_y0, d_z0, d_x, d_y, d_z,
    ...
    function stage1(p::ADKEStep, d_idx, d_x0, d_y0, d_z0, d_x, d_y, d_z,
               d_u0, d_v0, d_w0, d_u, d_v, d_w, d_e0, d_e, d_au, d_av,
    ...
    function stage2(p::ADKEStep, d_idx, d_x0, d_y0, d_z0, d_x, d_y, d_z,
               d_u0, d_v0, d_w0, d_u, d_v, d_w, d_e0, d_e, d_au, d_av,
               d_aw, d_ae, dt)
...
###############################################################################
# `TwoStageRigidBodyStep` struct
###############################################################################
    struct TwoStageRigidBodyStep <: IntegratorStep end
    ...
    """
    function initialize(p::TwoStageRigidBodyStep, d_idx, d_x, d_y, d_z, d_x0, d_y0, d_z0,
    ...
    function stage1(p::TwoStageRigidBodyStep, d_idx, d_x, d_y, d_z, d_x0, d_y0, d_z0,
    ...
    function stage2(p::TwoStageRigidBodyStep, d_idx, d_x, d_y, d_z, d_x0, d_y0, d_z0,
               d_u, d_v, d_w, d_u0, d_v0, d_w0, d_au, d_av, d_aw,
               dt)
...
###############################################################################
# `OneStageRigidBodyStep` struct
###############################################################################
    struct OneStageRigidBodyStep <: IntegratorStep end
    """Simple one stage rigid-body motion """
    function initialize(p::OneStageRigidBodyStep, d_idx, d_x, d_y, d_z, d_x0, d_y0, d_z0,
                   d_u, d_v, d_w, d_u0, d_v0, d_w0)
    ...
    function stage1(p::OneStageRigidBodyStep, d_idx, d_x, d_y, d_z, d_x0, d_y0, d_z0,
                d_u, d_v, d_w, d_u0, d_v0, d_w0, d_au, d_av, d_aw,
                dt)
    end

    function stage2(p::OneStageRigidBodyStep, d_idx, d_x, d_y, d_z, d_x0, d_y0, d_z0,
                d_u, d_v, d_w, d_u0, d_v0, d_w0, d_au, d_av, d_aw,
                dt)
        # update velocities
        d_u[d_idx] += dt * d_au[d_idx]
...
###############################################################################
# `VerletSymplecticWCSPHStep` struct
###############################################################################
    struct VerletSymplecticWCSPHStep <: IntegratorStep end
    ...
    function initialize(p::VerletSymplecticWCSPHStep) end

    function stage1(p::VerletSymplecticWCSPHStep, d_idx, d_x, d_y, d_z, d_u, d_v, d_w, dt)
    ...
    function stage2(p::VerletSymplecticWCSPHStep, d_idx, d_x, d_y, d_z, d_ax, d_ay, d_az,
...
###############################################################################
# `VelocityVerletSymplecticWCSPHStep` struct
###############################################################################
    struct VelocityVerletSymplecticWCSPHStep <: IntegratorStep end
    ...
    function initialize(p::VelocityVerletSymplecticWCSPHStep) end

    function stage1(p::VelocityVerletSymplecticWCSPHStep, d_idx, d_u, d_v, d_w, d_au, d_av, d_aw, dt)
    ...
    function stage2(p::VelocityVerletSymplecticWCSPHStep, d_idx, d_x, d_y, d_z, d_u, d_v, d_w,
               d_au, d_av, d_aw, dt)
...
###############################################################################
# `InletOutletStep` struct
###############################################################################
    struct InletOutletStep <: IntegratorStep end
    """A trivial integrator for the inlet/outlet particles
    """
    function initialize(p::InletOutletStep) end

    function stage1(p::InletOutletStep, d_idx, d_x, d_y, d_z, d_u, d_v, d_w, dt)
        dtb2 = 0.5*dt
        d_x[d_idx] += dtb2 * d_u[d_idx]
        d_y[d_idx] += dtb2 * d_v[d_idx]
        d_z[d_idx] += dtb2 * d_w[d_idx]
    ...
    function stage2(p::InletOutletStep, d_idx, d_x, d_y, d_z, d_u, d_v, d_w, dt)
        dtb2 = 0.5*dt
        d_x[d_idx] += dtb2 * d_u[d_idx]
        d_y[d_idx] += dtb2 * d_v[d_idx]
        d_z[d_idx] += dtb2 * d_w[d_idx]
...
###############################################################################
    struct LeapFrogStep <: IntegratorStep end
    ...
    function stage1(p::LeapFrogStep, d_idx, d_x, d_y, d_z, d_u, d_v, d_w, d_ax, d_ay, d_az,
               dt)
    ...
    function stage2(p::LeapFrogStep, d_idx, d_x, d_y, d_z, d_u, d_au, d_v, d_av,
...
###############################################################################
    struct PEFRLStep <: IntegratorStep end
    ...
    function stage1(p::PEFRLStep, d_idx, d_x, d_y, d_z, d_u, d_v, d_w, d_ax, d_ay,
               d_az, dt)
    ...
    function stage2(p::PEFRLStep, d_idx, d_x, d_y, d_z, d_u, d_au, d_v, d_av,
               d_w, d_aw, d_ax, d_ay, d_az,
               d_rho, d_arho, d_e, d_ae, dt=0.0)

        lamda = -0.2123418310626054
        fac = (1.0 - 2.0*lamda) / 2.0
    ...
    function stage3(p::PEFRLStep, d_idx, d_x, d_y, d_z, d_u, d_au, d_v, d_av,
               d_w, d_aw, d_ax, d_ay, d_az,
               d_rho, d_arho, d_e, d_ae, dt=0.0)
    ...
    function stage4(p::PEFRLStep, d_idx, d_x, d_y, d_z, d_u, d_au, d_v, d_av,
               d_w, d_aw, d_ax, d_ay, d_az,
               d_rho, d_arho, d_e, d_ae, dt=0.0)
    ...
    function stage5(p::PEFRLStep, d_idx, d_x, d_y, d_z, d_u, d_au, d_v, d_av,
               d_w, d_aw, d_ax, d_ay, d_az,
               d_rho, d_arho, d_e, d_ae, dt=0.0)
